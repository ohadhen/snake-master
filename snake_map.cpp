...
void SnakeMap::redrawPreview(void)
{
    clear_map(this->map_array);
    for (int i = 0; i < MAP_END; i++)
    {
    ...
    for (int i = 0; i < MAP_HEIGHT; i++)
    {
        for (int j = 0; j < MAP_WIDTH; j++)
        {
            for (int indx = 0; indx < snake->previewSteps.size(); indx++) 
            {
                cout << "\033[32m" << endl;

                cout << map_array[snake->previewSteps[indx].first][snake->previewSteps[indx].second] << " ";
